...
public class ArticlePage<T> {
	...
	public ArticlePage(int total, int currentPage, int size, List<T> content
		...
		//4. 페이징 블록 시작 /////////
		// 이전/다음 계산
		int prevPage = (this.currentPage > 1) ? this.currentPage - 1 : 1;
		int nextPage = (this.currentPage < this.totalPages) ? this.currentPage + 1 : this.totalPages;

		pagingArea += "<div class='pagination w-page dataTables_paginate paging_simple_numbers' id='example2_paginate'>";

		// 이전 버튼
		pagingArea += "<a class='page-navi prev clsPagingArea";
		if (this.currentPage < 2) {
		    pagingArea += " disabled";
		}
		pagingArea += "' href='#' data-current-page='" + prevPage + "' data-keyword='" + this.keyword + "'>Prev</a>";

		// 페이지 번호 링크
		pagingArea += "<div class='page-links'>";
		for (int pNo = this.startPage; pNo <= this.endPage; pNo++) {
		    pagingArea += "<a class='page-link clsPagingArea";
		    if (this.currentPage == pNo) {
		        pagingArea += " active";
		    }
		    pagingArea += "' href='#' data-current-page='" + pNo + "' data-keyword='" + this.keyword + "'>" + pNo + "</a>";
		}
		pagingArea += "</div>";

		// 다음 버튼
		pagingArea += "<a class='page-navi next clsPagingArea";
		if (this.currentPage >= this.totalPages) {
		    pagingArea += " disabled";
		}
		pagingArea += "' href='#' data-current-page='" + nextPage + "' data-keyword='" + this.keyword + "'>Next</a>";
