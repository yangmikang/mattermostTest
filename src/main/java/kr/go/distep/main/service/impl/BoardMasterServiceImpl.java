...
public class BoardMasterServiceImpl implements BoardMasterService {
    ...
    public int boardInsert(BoardVo boardVo) {
        try {
            ...
            boardVo.setFileGroupNo(fileGroupNo); // 단 한 번만 지정

            // 2. 썸네일이 있으면 append 방식으로 추가
            ...
            if (uploadFiles != null && uploadFiles.length > 0 && uploadFiles[0].getOriginalFilename().length() > 0) {
                uploadService.appendFilesToGroup(fileGroupNo, uploadFiles, "GENERAL");
            }
