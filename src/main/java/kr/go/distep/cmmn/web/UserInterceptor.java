	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws IOException, Exception {
	    ...
	    if (session != null) {
	        ...
	        if (userObj != null) {
	            ...
	            apprYn = ((UserVO) userObj).getAppr_yn();
	            cityCode = ((UserVO) userObj).getCity_code();
	            if ("Y".equals(apprYn)) {
	    ...
	    log.setRequestUri(fullUri);
	    log.setLogTime(logTime);
	    log.setCityCode(cityCode);
	    log.setRole(userType);

	    logService.insertRequestLog(log);

	    String boardMasterCode = userService.getBoardMasterCodeByLink(fullUri);

	    // 상세페이지일 경우 역매핑하여 boardMasterCode 가져오기 
	    if ((boardMasterCode == null || boardMasterCode.isEmpty())
	            && requestUri.contains("/board/detail.do")) {
	        String boardCode = request.getParameter("boardCode");
	        if (boardCode != null && !boardCode.isEmpty()) {
	            boardMasterCode = userService.getBoardMasterCodeByBoardCode(boardCode);
	           // log.info("boardCode {}  boardMasterCode {}", boardCode, boardMasterCode);
	        }
	    }

	    //  공개 페이지 여부
	    boolean isPublic = false;
		if (requestUri.contains("/calendar/detail.do") /* && requestUri.contains("/calendarCreate.do") */) {
	        isPublic = true;
	    } else if (boardMasterCode != null && !boardMasterCode.isEmpty()) {
	        HashMap<String, String> gparam = new HashMap<>();
	        gparam.put("userType", "N");
	        gparam.put("boardMasterCode", boardMasterCode);
	        String gAuth = userService.getBoardReadAuth(gparam);
	        isPublic = "Y".equals(gAuth);
	    }

	    //  세션 만료 처리 
	    if ("".equals(userType) && !isPublic
	        && !requestUri.endsWith("/login.do")
	        && !requestUri.endsWith("/logout.do")
	        && !requestUri.endsWith("/index.do")
	        && !requestUri.endsWith("/userLogin.do")) {
	        ...
	        if (isAjax) {
	            ...
	            response.getWriter().write("{\"success\":false,\"reason\":\"SESSION_EXPIRED\"}");
	        } else {
	            response.setContentType("text/html; charset=UTF-8");
	            PrintWriter out = response.getWriter();
	            out.println("<script>");
	            out.println("alert('Session has expired. Redirecting to the login page.');");
	    ...
	    //  권한 체크
	    String authUserType = userType;
	    if ("".equals(userType) ) {
	        authUserType = "N";
	    }

	    HashMap<String, String> param = new HashMap<>();
	    param.put("userType", authUserType);
	    param.put("boardMasterCode", boardMasterCode);

	    boolean skipAuth = ("".equals(userType) || "N".equals(userType)) && isPublic;

	    if (!skipAuth && !"A".equals(userType)) {
	        if (boardMasterCode != null && !boardMasterCode.isEmpty()) {
	            String authYn = userService.getBoardReadAuth(param);
	            if (authYn == null || "N".equals(authYn)) {
	                ...
	                if (isAjax) {
	                    ...
	                    response.setContentType("application/json;charset=UTF-8");
	                    response.getWriter().write("{\"success\":false,\"reason\":\"NO_AUTH\"}");
	                } else {
	                    response.setContentType("text/html; charset=UTF-8");
	                    PrintWriter out = response.getWriter();
	                    out.println("<script>");
	                    out.println("alert('You do not have permission to access this page.');");
