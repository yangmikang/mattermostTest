                                                                        ...
                                                                        <li>
	                                                                        <div class="in" style="width: 100%;display: flex;justify-content: space-between;">
	                                                                         <%--    <img class="img-fluid imgFileSaveLocate" style="display: none;" src="<c:url value='${fileDetailVO.fileSaveLocate}' />" data-file-original-name="${fileDetailVO.fileOriginalName}" data-file-save-locate="${fileDetailVO.fileSaveLocate}" />
	                                                                          --%>  
	                                                                           <p class="tit" onclick="fn_dw(this)" title="${fileDetailVO.fileOriginalName} [${fileDetailVO.fileExt}, ${fileDetailVO.fileFancySize}]"  data-file-save-locate="${fileDetailVO.fileSaveLocate}" style="cursor: pointer;">${fileDetailVO.fileOriginalName} [${fileDetailVO.fileExt}, ${fileDetailVO.fileFancySize}]
	                                                                            </p>
	                                                                            <div class="btn-wrap">
	                                                                            </div>
    	...
    	if (boxGroup && boxGroup.children.length > 0) {
         ...
         function handleFileUpload(event) {
       	  ...
       	  for (let i = 0; i < files.length; i++) {
       	    ...
       	    const rpext = '.' + extension;
       	    const fileName = file.name.replace(rpext, '');
       	    ...
       	    if (file.size > 20 * 1024 * 1024) {
       	      ...
       	      li.innerHTML =
       	        '<div class="in">' +
       	          ...
       	          '</div>' +
       	        '</div>' +
       	        '<p class="file-hint" style="display:flex;width:100%;justify-content:flex-start;">Only up to 20MB per file is allowed.</p>';
       	      fileListArea.appendChild(li);
       	      continue;
       	    }
       	    ...
       	    const li = document.createElement('li');
       	    li.dataset.fullname = file.name;   
			li.dataset.size = String(file.size); 
       	    ...
       	    li.innerHTML =
       	      '<div class="in">' +
       	        '<div class="file-name">' + fileName + ' [' + extension + ', ' + formatBytes(file.size) + ']</div>' +
       	        '<div class="file-btn">' +
       	          '<button type="button" class="btn btn-txt ico-before ico-del sm h-auto" style="border:none;" onclick="deleteSingleFile(' + index + ', this)">Delete</button>' +
       	        '</div>' +
       	      '</div>';
          ...
          function finalizeUpload(input) {
            try { updateFileCount(); } catch (e) {}
         ...
         function deleteSingleFile(index, button, fileId = null) {
		  ...
		  if (fileId !== null && fileId !== undefined) {
		    ...
		    const hiddenInput = document.querySelector(
		      `input[name="existingFileIds"][value="${fileId}"]`
		    );
		    if (hiddenInput) hiddenInput.remove();
			  ...
			  if (fullname && fsize) {
			    ...
			    var fileNameEl = li ? li.querySelector('.file-name') : null;
			    var nameFromText = '';
			    if (fileNameEl && fileNameEl.textContent) {
			      nameFromText = fileNameEl.textContent.split(' [')[0];
			    }
			    for (var j = 0; j < uploadedFiles.length; j++) {
			      var ff = uploadedFiles[j];
			      if (ff && (ff.name === nameFromText || ff.name.replace(/\.[^.]+$/, '') === nameFromText)) {
			        fileIndex = j;
			        break;
			      }
			    }
