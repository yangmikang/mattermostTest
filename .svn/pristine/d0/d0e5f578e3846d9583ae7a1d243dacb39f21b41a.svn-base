...
public class BoardController {
    ...
    public String detail(BoardVo boardVo, Model model, HttpSession session,
        ...
        if (user != null) {
        	...
        	log.info("Fefefffffffffffffffff로직타나");
        	userType = "N";
        	 param.put("userType", userType);
             param.put("boardMasterCode", boardMasterCode);
	...
	@ResponseBody
	public Map<String, Object> updateCm(
	        BoardVo boardVo,
	    ...
	    try {
	        ...
	        if (result > 0) {
	            ...
	            res.put("error", "NO_ROWS_UPDATED");
	            res.put("message", "Update failed: No rows updated.");
	            return res;
	        }

	    } catch (org.springframework.dao.DataIntegrityViolationException e) {// 컬럼 length오류 
	        resp.setStatus(400);                   
	        res.put("success", false);
	        res.put("error", "DATA_TOO_LONG");
	        res.put("message", "제목/컬럼 길이 제한 초과");
	        return res;
