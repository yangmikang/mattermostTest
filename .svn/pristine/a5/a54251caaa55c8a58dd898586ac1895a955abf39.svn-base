...
$(function(){
	$('#checkEmailBtn').click(function() { 
	    ...
	    $.ajax({
	        ...
	        success: function(response) {
	        	if (response === 'Y') {
	        	    $('#emailCheckResult').html('<p class="form-hint info">This email is already in use. Please enter a different email.</p>').css('color', 'red');
	        	} else {
...
$(document).ready(function() {
	...
	$('#userId').on('input', function() {
	    ...
	    if (regex.test(val)) {
	        alert("Please use only letters and numbers for your ID.");
	    }
...
function validateForm() {
    ...
    if (city_code === "") {
        alert("Please select your Local Government Name.");
        return false;
    }

    if (city_code === "99") {
        if (city_etc === "") {
            alert("Please enter your other Local Government Name.");
            return false;
        }
							...
							<div class="txt-box bg-white-login">
								<form action="${pageContext.request.contextPath}/register.do" method="post" onsubmit="return validateForm();">
									<div class="fieldset">
										...
										<div class="form-group margin-b">
											<div class="form-tit">
												<label for="">Local Government Name</label>
											</div>
											<div class="form-conts btn-ico-wrap">
												<select class="form-select md" title="선택" name="city_code" id="city_code"> 
									        		<option value="">---Select Name---</option>
									        		<option value="01">City of Daejeon</option>
									        		<option value="02">City of Dortmund</option>
									        		<option value="03">City of Málaga</option>
									        		<option value="04">County of Montgomery, MD</option>
									        		<option value="05">City of Seattle</option>
									        		<option value="06">Province of Québec</option>
<!-- 									        		<option value="99">Other city</option> -->
									        		<option value="07">GINI Secretariat</option>
									        	</select>
											</div>
