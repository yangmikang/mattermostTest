			...
			<div class="conts-wrap">
				<div class="conts-wrap log-in-box">
					<div class="txt-box bg-white-login">
						...
						<h2 class="section-title">Find Your ID</h2>

						<c:if test="${not empty errorMessage}">
							<p class="error-message">${errorMessage}</p>
						</c:if>

						<form action="findId.do" method="post" class="fieldset">
							<div class="form-group">
								<label for="user_name">Name</label> <input type="text" name="name" id="user_name" class="form-control" placeholder="Enter your name" required />
								<label for="user_email">Email</label> <input type="email" name="email" id="user_email" class="form-control" placeholder="Enter your email" required />
							</div>
							<div class="comp-btn-wrap"> 
								<button type="submit" class="btn primary">Find ID</button>
							</div> 
						</form>
				</div>
			</div>  
