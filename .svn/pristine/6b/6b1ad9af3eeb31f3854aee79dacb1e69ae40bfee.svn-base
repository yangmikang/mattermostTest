...
@media (min-width: 600px) and (max-width: 1023px) {
    #container > .inner .gini-cont-wrap .gini-cont-box .cont50{
        width: 100%;
    } 
}
.upload-list > li {
				 ...
				 <div class="gini-cont-wrap">
						...
						<div class="row cont100">
							...
							<div class="cont-left cont50">
								...
								<div class="form-group">
									...
									<div class="form-conts">
										<input type="text" id="consult_name" maxlength="150"  class="form-control" placeholder="Event Title">
									</div>
								</div>
							...
							<div class="cont-right cont50">
								...
								<div class="form-group">
									...
									<div class="form-conts">
										<input type="text" maxlength="10000" id="description" class="form-control" placeholder="Event Description">
									</div>
								</div>
								<!-- //폼그룹 -->
								<!-- 폼그룹 -->
								<div class="form-group">
									...
									<div class="form-conts">
										<select class="form-select lg" id="selectOrg" title="Select Organizer">
					...
					<div class="gini-cont-box">
						...
						<div class="page-btn-wrap both" >
							...
							<div>
							<button type="button" class="btn lg tertiary"onclick="registerEvent()">Create Event</button>
							</div>
						</div>
...
<script>
  document.addEventListener('DOMContentLoaded', function(){
    var tz = (Intl && Intl.DateTimeFormat().resolvedOptions().timeZone) || 'Asia/Seoul';
    var el = document.getElementById('clientTimeZone');
    if (el) el.value = tz;
  });
</script>
...
<script>
/*   document.getElementById('clientTimeZone').value =
    Intl.DateTimeFormat().resolvedOptions().timeZone; // e.g. "Asia/Seoul"
  // 전역 파일 저장 배열 */
  let uploadedFiles = [];
  ...
  function deleteAllFiles() {
	    uploadedFiles = [];
	    var ul = document.getElementById('fileListArea');
	    if (ul) ul.innerHTML = '';
	    updateFileCount();
	  }
  ...
  function handleFileUpload(event) {
	    ...
	    var ul = document.getElementById('fileListArea');
	    var allowed = ['pdf','doc','docx','xls','xlsx','ppt','pptx','txt','jpg','jpeg','png','gif','zip','hwp'];

	    var capacity = 6 - uploadedFiles.length;
	    for (var i = 0; i < files.length; i++) {
	      var f = files[i];
	      var ext = f.name.split('.').pop().toLowerCase();
	      var base = f.name.slice(0, -(ext.length + 1)); 

	      if (allowed.indexOf(ext) === -1) {
	        var liErr1 = document.createElement('li');
	        liErr1.className = 'is-error';
	        liErr1.innerHTML =
	          '<div class="in">' +
	            '<div class="file-name">' + base + ' [' + ext + ', ' + formatBytes(f.size) + ']</div>' +
	            '<div class="file-btn">' +
	            ...
	            '</div>' +
	          '</div>' +
	          '<p class="file-hint">Only the following file types can be uploaded:<br>pdf, doc, docx, xls, xlsx, ppt, pptx, txt, jpg, jpeg, png, gif, zip, hwp</p>';
	        ul.appendChild(liErr1);
	        continue;
	      }

	      if (f.size > 20 * 1024 * 1024) {
	        var liErr2 = document.createElement('li');
	        liErr2.className = 'is-error';
	        liErr2.innerHTML =
	          '<div class="in">' +
	            '<div class="file-name">' + base + ' [' + ext + ', ' + formatBytes(f.size) + ']</div>' +
	            '<div class="file-btn">' +
	      ...
	      li.dataset.fullname = f.name;
	      li.dataset.size = String(f.size);
	      li.innerHTML =
	        '<div class="in">' +
	          '<div class="file-name">' + base + ' [' + ext + ', ' + formatBytes(f.size) + ']</div>' +
	          '<div class="file-btn">' +
  ...
  function deleteUploadedFile(btn) {
	    ...
	    var fullname = li && li.dataset ? li.dataset.fullname : '';
	    var sizeStr = li && li.dataset ? li.dataset.size : '0';
	    var sizeNum = Number(sizeStr);

	    for (var i = 0; i < uploadedFiles.length; i++) {
	      var f = uploadedFiles[i];
	      if (f && f.name === fullname && f.size === sizeNum) {
  ...
  function registerEvent() {
	...
	// 유효성 검사
	    ...
	    if (description.length > 10000) {
	        alert("The description must be no more than 10000 characters.");
	        return false;
	    }
	  ...
	  axios.post(contextPath + '/calendar/register.do', formData, {
		...
		.then(function (res) {
		  ...
		  if (res.data && res.data.success) {
		    ...
		    location.href = contextPath + (res.data.redirectUrl || '/calendar.do');
		  } else {
		    alert((res.data && res.data.message) ? res.data.message : 'Unable to register the event. Please try again later.');
		    location.href = contextPath + '/calendar.do';
		    if (res.data && res.data.redirectUrl) {
		      location.href = contextPath + res.data.redirectUrl;
		    }
		  }
		})
		.catch(function (err) {
		  ...
		  alert('Unable to register the event. Please try again later.');
		  location.href = contextPath + '/calendar.do';
		});
