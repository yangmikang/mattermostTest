				...
				<div class="inner">
					...
					<nav class="breadcrumb-wrap" aria-label="브레드크럼">
						<ol class="breadcrumb">
							...
							<li><a href="#" class="txt">My Page</a></li>
						</ol>
					</nav>
					<!-- breadcrumb -->
					<!-- 페이지 타이틀 영역 -->
					<div class="visual-area">
						<h2 class="h-tit">My Page</h2>
...
<%-- 						    <c:if test="${user.city_code == '07'}"> --%>
<!-- 						    	<input type="hidden" name="city_code" value="07"/>     -->
<%-- 						    </c:if> --%>
<%-- 							<c:if test="${user.city_code != '07'}">   --%>
							    <div class="form-group margin-b">
							        <div class="form-tit">
							            <label for="city_code">Local Government Name</label>
							        </div>
							        <div class="form-conts btn-ico-wrap">
							            <select class="form-select md" name="city_code" id="city_code">
							                <option value="">---Select Name---</option>
							                <option value="01" ${user.city_code == '01' ? 'selected' : ''}>City of Daejeon</option>
							                <option value="02" ${user.city_code == '02' ? 'selected' : ''}>City of Dortmund</option>
							                <option value="03" ${user.city_code == '03' ? 'selected' : ''}>City of Málaga</option>
							                <option value="04" ${user.city_code == '04' ? 'selected' : ''}>County of Montgomery, MD</option>
							                <option value="05" ${user.city_code == '05' ? 'selected' : ''}>City of Seattle</option>
							                <option value="06" ${user.city_code == '06' ? 'selected' : ''}>Province of Québec</option>
<%-- 							                <option value="99" ${user.city_code == '99' ? 'selected' : ''}>Other city</option>  --%>
							                <option value="07" ${user.city_code == '07' ? 'selected' : ''}>GINI Secretariat</option>
							            </select>
							        </div>
							    ...
							    <div class="form-group margin-b">
							        ...
							        <div class="form-conts btn-ico-wrap">
							        	<c:if test="${user.role eq 'A'}">Administrator</c:if>
							        	<c:if test="${user.role eq 'G'}">General</c:if>
							        	<c:if test="${user.role eq 'C'}">Government</c:if>
							        	<c:if test="${user.role eq 'O'}">Institution</c:if>
									</div>
							    </div>
<%-- 						    </c:if> --%>
