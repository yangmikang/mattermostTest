...
.thumb {
  ...
  width: 100%;
  height: 200px;
  background: center/cover no-repeat url('${pageContext.request.contextPath}/img/component/common/no-image-b.png');
  border-radius: 8px;
  overflow: hidden;
  cursor: pointer;
}
...
.card-img {
  ...
  background-image: url('../../img/component/common/no-image-ba.png');
        ...
        <div class="search-top-box">
          <div class="sch-form-wrap flex">
            <div class="search-box">
	            <div class="sch-input">
	              <input type="text" class="form-control searchKeyword" id="searchKeyword" placeholder="Enter Search keyword" title="Enter Search keyword">
	              <button type="button" class="btn btn-ico ico-sch" id="search"><span class="sr-only">검색</span></button>
	            </div>
	            <button type="button" class="btn ico-refresh lg tertiary" onclick="resetSearch()" title="Reset"></button>
            </div>
            
            <c:if test="${userYn == 'Y' or userType == 'A'}">
          	 	 <button type="button" class="btn btn-txt primary" style="width: 150px;" onclick="location.href='${pageContext.request.contextPath}/board/boardPage.do?boardMasterCode=${boardMasterCode}'">Create Post</button>
			</c:if>
          </div>
        </div>
        <div class="search-list-top type1">
          <div class="sch-info" role="region" aria-live="polite">
            Total <span class="keyword">0</span>
          </div>
        </div>
  ...
  document.addEventListener("DOMContentLoaded", function () {
	    ...
	    switch (boardMasterCode) {
	      ...
	      case "A01":
	        ...
	        mdl.innerText = mdl.innerText ="About GINI";
	        descEl.innerText = "Discover events and initiatives from action groups.";
	        break;
	      default:
	        titleEl.innerText = breadcrumbMenuEl.innerText = "GINI News";
	    ...
	    // 초기 리스트 로딩
		  ...
		  document.getElementById("search").addEventListener("click", function () {
			...
			// 현재 게시글이 하나도 없으면 검색 막기
			/*   const listItems = document.querySelectorAll(".stb-list .li, .thumbnail-list .li");
			  const hasRealItems = Array.from(listItems).some(function(el) {
				  return !el.classList.contains("no-data");
				});
			  if (!hasRealItems) {
			    alert("There are no posts to search.");
			    return;
			  } */
			  // 공백만 입력한 경우 결과 비우고 리턴
			  if (keyword === '') {
			 /*    document.querySelector(".keyword").innerText = '0';
			    const boardMasterCode = window.boardMasterCode || 'N01';
			    const isThumbnailType = ["N01", "N02", "N03", "A01"].includes(boardMasterCode);
			    ...
			    const targetList = isThumbnailType
			    ? document.querySelector(".thumbnail-list")
			    : document.querySelector(".stb-list");
	 */
				/*   targetList.style.display = isThumbnailType ? 'grid' : 'block';
				  targetList.insertAdjacentHTML('afterend', '<li class="li no-data"><p>There are no posts.</p></li>'); */
				 alert("Please enter a keyword.")
		    	  /* document.querySelector("#divPagingArea").innerHTML = '<li class="li no-data" <p>There are no posts.</p></li>'; */
	...
	function resetSearch(){
	 ...
	 let search = document.querySelector("#searchKeyword")
	  ...
	  axios.post(`${pageContext.request.contextPath}/board/listAjax.do`, data)
	    .then(function (res) {
	      ...
	      if (list.length === 0) {
	    	  ...
	    	  document.querySelector(".nodata").innerHTML = '<li class="li no-data" <p>There are no posts.</p></li>';
	    	  ...
	    	  // item 사용
		        ...
		        if (["N01", "N02", "N03","A01"].includes(boardMasterCode)) {
		        	...
		        	if (firstFile && firstFile.fileSaveLocate) {
		        	    ...
		        	    thumbnailPath = baseContextPath + '../../img/component/common/no-image-b.png';
		        	} 
		        	html += '<li class="li">'
		        		+ '<div class="thumb" onclick="location.href=\'' + detailUrl + '\'">'
